...
| `FLOCI_SERVICES_LAMBDA_DOCKER_FLAGS` | _(none)_ | Extra flags for every Lambda container: `-e`/`--env`, `-v`/`--volume`, `-p`/`--publish`, `--add-host`, `--network`, `-u`/`--user`, `--dns`, and `--privileged`. The LocalStack-compatible `LAMBDA_DOCKER_FLAGS` alias is also accepted |
